...
COMMAND = '''awk -F"\t" 'OFS="\t" {print "chr"$158, $159, $4, $1}' brca1_multianno_processed | grep -v "ID" | grep -v "START" > mydata.bed'''
subprocess.call(COMMAND, shell=True)
print("Bedfile created")

#Next, perform bedtools intersect to create input file for pfam to merge with brca1_multianno_processed 
...
data1 = pd.read_table('brca1_multianno_processed', sep='\t')
df = data1.copy()
...
#save file
pd.DataFrame(shuffled_new).to_csv('../../BRCA1/pipeline.csv', index=False)
