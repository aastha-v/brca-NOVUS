...
input_filename='brca1.vcf'
preprocessing_foldername=$cwd/brca-NOVUS/preprocessing/
...
#create op_outcome
awk -F"\t" 'OFS="\t" {print $1":"$2":"$4":"$5}' sorted.vcf | grep -v "#" | awk 'BEGIN{print "ID\tOutcome"}1' - > op_outcome
...
#run annovar
perl ../brca-NOVUS/scripts/convert2annovar.pl --format vcf4 $input_filename --outfile res.avinput --includeinfo --withzyg

perl ../brca-NOVUS/scripts/table_annovar.pl *.avinput ../humandb --buildver hg38 --outfile res \
...
python3 ../scripts/pipeline_brca1.py
